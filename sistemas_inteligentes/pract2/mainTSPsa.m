...
for i = 1:length(archivos)
    ...
    [best_tour, best_dist, log_distancias] = solve_tsp_sa(coords, tiempo_maximo);
    ...
    subplot(1, 2, 1);
    plot(log_distancias, 'b-', 'LineWidth', 2);
    ...
    subplot(1, 2, 2);
    plot(coords(:,1), coords(:,2), 'ko', 'MarkerFaceColor', 'r', 'MarkerSize', 5); hold on;
    
    ruta_coords = coords(best_tour, :);
    ruta_coords =[ruta_coords; ruta_coords(1, :)]; 
    plot(ruta_coords(:,1), ruta_coords(:,2), 'b-', 'LineWidth', 1.5);
